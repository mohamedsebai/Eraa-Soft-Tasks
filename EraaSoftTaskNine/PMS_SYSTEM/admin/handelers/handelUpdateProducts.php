    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){ 
        if(isset($_POST['updateProduct'])){
            ...
            if( $img_error == 4){
                    ...
                    $new_img_name = $row['prd_image']; // insert old image if there is no update for image 
            ...
            if(empty($prd_quantity)){
                $form_errors[] = 'Quantity can no be empty if there is no prd_quantity make it equal zero';
            }
            ...
            if(empty($form_errors)){
                if(updateProductData($prd_name, $prd_quantity, $prd_price, $prd_brand_name, $prd_discount, $prd_size, $new_img_name, $category_id ,$id)){
                    $_SESSION['success_update'] = 'data update successfully!';
                    header("Location: ../update_product.php?product_id=$id");
                }else{
                    $_SESSION['update_product_database_error'] = 'try agian later!';
                    header("Location: ../update_product.php?product_id=$id");
                    exit();
                }
                ?>
